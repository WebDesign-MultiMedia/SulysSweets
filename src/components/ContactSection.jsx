  ...
  return (
    <section id="contact" className="bg-cream px-5 py-20 sm:px-8 sm:py-28">
      <div className="mx-auto max-w-3xl text-center">
        ...
        <Reveal delay={100}>
          <h2 className="mt-4 font-display text-4xl text-plum sm:text-5xl">{t.contact.title}</h2>
        </Reveal>
        ...
        >
          <>
            <p className="text-xs font-medium tracking-[0.3em] text-mauve-dark uppercase">
              {t.contact.orderInfoTitle}
            </p>
            <ul className="mt-4 grid gap-2 text-sm text-plum/80 sm:grid-cols-2">
              {t.contact.orderInfo.map((item) => (
                <li key={item}>{item}</li>
              ))}
            </ul>
          </>
